...
const SOURCE_MANIFEST_PATH = `${BASE_DIR}/spigen-note10-gomibo-case-sources.json`
const SOURCE_PROVENANCE_PATH = `${BASE_DIR}/spigen-note10-gomibo-case-asset-provenance.json`
const DERIVATION_PROVENANCE_PATH = `${BASE_DIR}/spigen-note10-gomibo-case-derived-provenance.json`
const OUTPUT_PATH = `${BASE_DIR}/spigen-note10-gomibo-case-review.json`
const MODEL_ID = 'galaxy-note-10'
...
assert(sourceManifest.candidates.length === 1, 'Expected exactly one Note 10 source candidate')
assert(sourceProvenance.assets.length === 1, 'Expected exactly one downloaded Note 10 source')
...
assert(derivation.results.length === 1, 'Expected exactly one Note 10 derivation result')
...
assert(source.publicationEligible, 'Note 10 source is not publication-eligible')
...
assert(result.alpha.cameraOpeningProfile.id === 'galaxy-note-10-upper-right-single', 'Strict Note 10 opening profile changed')
assert(result.alpha.cameraOpeningProfilePassed, 'Strict Note 10 opening profile failed')
assert(result.alpha.significantHoles.length === 1, 'Expected exactly one significant Note 10 opening')
...
const model = {
  ...
  officialDimensions: {
    ...
    directPageVerification: {
      ...
      verificationMethod: 'Verified SM-N970F and SM-N970FZSDBTU in the archived Samsung Ireland page, then read the exact Dimension (HxWxD, mm) item from Samsung api.samsung.com/model for SM-N970FZSDBTU.',
    },
  },
...
const review = {
  ...
  acceptanceCriteria: [
    ...
    'The strict Galaxy Note 10 profile finds exactly one significant upper-right opening and no unexpected significant holes.',
    ...
    'Physical dimensions come from the archived first-party Samsung page identity and Samsung model API response for SM-N970FZSDBTU.',
  ],
  summary: {
    sourceCandidates: 1,
