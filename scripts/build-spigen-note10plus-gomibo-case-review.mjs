...
const SOURCE_MANIFEST_PATH = `${BASE_DIR}/spigen-note10plus-gomibo-case-sources.json`
const SOURCE_PROVENANCE_PATH = `${BASE_DIR}/spigen-note10plus-gomibo-case-asset-provenance.json`
const DERIVATION_PROVENANCE_PATH = `${BASE_DIR}/spigen-note10plus-gomibo-case-derived-provenance.json`
const OUTPUT_PATH = `${BASE_DIR}/spigen-note10plus-gomibo-case-review.json`
const MODEL_ID = 'galaxy-note-10-plus'
...
assert(sourceManifest.candidates.length === 1, 'Expected exactly one Note 10 Plus source candidate')
assert(sourceProvenance.assets.length === 1, 'Expected exactly one downloaded Note 10 Plus source')
...
assert(derivation.results.length === 1, 'Expected exactly one Note 10 Plus derivation result')
...
assert(source.publicationEligible, 'Note 10 Plus source is not publication-eligible')
...
assert(result.alpha.cameraOpeningProfile.id === 'galaxy-note-10-plus-upper-right-single', 'Strict Note 10 Plus opening profile changed')
assert(result.alpha.cameraOpeningProfilePassed, 'Strict Note 10 Plus opening profile failed')
assert(result.alpha.significantHoles.length === 1, 'Expected exactly one significant Note 10 Plus opening')
...
const model = {
  ...
  visualReview: {
    status: 'accepted',
    criteria: 'The real empty shell is orthogonal and fully visible; its complete outer boundary and single camera-and-flash opening are directly segmented from the source photograph.',
  },
  officialDimensions: {
    ...dimensions,
    directPageVerification: {
      ...
      verificationMethod: 'Verified SM-N975F and SM-N975FZSDBTU in the archived Samsung Ireland page, then read the exact Dimension (HxWxD, mm) item from Samsung api.samsung.com/model for SM-N975FZSDBTU.',
    },
  },
...
const review = {
  ...
  acceptanceCriteria: [
    ...
    'The strict Galaxy Note 10 Plus profile finds exactly one significant upper-right opening and no unexpected significant holes.',
    ...
    'Physical dimensions come from the archived first-party Samsung page identity and Samsung model API response for SM-N975FZSDBTU.',
  ],
  summary: {
    sourceCandidates: 1,
