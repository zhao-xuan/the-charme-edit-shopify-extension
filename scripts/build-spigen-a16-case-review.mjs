...
const SOURCE_MANIFEST_PATH = `${BASE_DIR}/spigen-a16-case-sources.json`
const SOURCE_PROVENANCE_PATH = `${BASE_DIR}/spigen-a16-case-asset-provenance.json`
const DERIVATION_PROVENANCE_PATH = `${BASE_DIR}/spigen-a16-case-derived-provenance.json`
const OVERLAY_PATH = `${BASE_DIR}/reviews/galaxy-a16-spigen-real-shell-geometry-overlay.png`
const OUTPUT_PATH = `${BASE_DIR}/spigen-a16-case-review.json`
const EXPECTED = {
  modelId: 'galaxy-a16',
...
async function assertSourceAsset(asset) {
  ...
  assert(sha256(bytes) === asset.encodedSha256, 'A16 source encoded SHA-256 changed')
  const decoded = await sharp(bytes).removeAlpha().raw().toBuffer({ resolveWithObject: true })
  assert(decoded.info.width === asset.width && decoded.info.height === asset.height, 'A16 source dimensions changed')
  assert(decoded.info.channels === 3, 'A16 source channels changed')
  assert(sha256(decoded.data) === asset.decodedPixelSha256, 'A16 source pixels changed')
  return bytes
}
...
async function writeReviewOverlay(sourceBytes, crop, alpha, width, height) {
  const source = await sharp(sourceBytes).extract(crop).removeAlpha().raw().toBuffer({ resolveWithObject: true })
  assert(source.info.width === width && source.info.height === height, 'A16 overlay crop dimensions changed')
  ...
  return {
    path: OVERLAY_PATH,
    sourceFramePath: SOURCE_PROVENANCE_PATH,
...
assert(sourceManifest.targetModelId === EXPECTED.modelId, 'A16 manifest model ID changed')
assert(sourceManifest.targetModelName === EXPECTED.modelName, 'A16 manifest model name changed')
assert(sourceManifest.sourceModelId === EXPECTED.sourceModelId, 'A16 source model changed')
...
assert(sourceManifest.officialDimension.supportModel === EXPECTED.supportModel, 'Samsung support model changed')
assert(sourceManifest.officialDimension.reportedValue === EXPECTED.reportedValue, 'Samsung dimensions changed')

assert(sourceProvenance.assets.length === 1, 'Expected exactly one verified A16 source')
assert(sourceProvenance.summary.catalogIdentitiesVerified === 1, 'A16 catalog identity verification failed')
assert(sourceProvenance.summary.productRecordsVerified === 1, 'Spigen product verification failed')
assert(sourceProvenance.summary.repeatedSourceFetchesByteIdentical === 1, 'Repeated Spigen image bytes differ')
assert(sourceProvenance.summary.sourceImagesPixelVerified === 1, 'Spigen source pixel verification failed')
assert(sourceProvenance.summary.officialDimensionsVerified === 1, 'Samsung dimension verification failed')
assert(sourceProvenance.summary.publicationEligible === 1, 'A16 source is not publication-eligible')
assert(derivation.results.length === 1, 'Expected exactly one A16 derivation result')
assert(derivation.summary.thresholdStabilityPassed === 1, 'A16 threshold stability failed')
assert(derivation.summary.cameraOpeningQaPassed === 1, 'A16 camera opening QA failed')
assert(derivation.summary.exactPairAlpha === 1, 'A16 Black and White alpha differs')
assert(derivation.summary.automatedQaPassed === 2, 'Both A16 candidates must pass automated QA')

const asset = sourceProvenance.assets[0]
const result = derivation.results[0]
assert(asset.targetModelId === EXPECTED.modelId && result.modelId === EXPECTED.modelId, 'Derived A16 identity changed')
assert(asset.targetModelName === EXPECTED.modelName && result.modelName === EXPECTED.modelName, 'Derived A16 name changed')
assert(asset.sourceModelId === EXPECTED.sourceModelId && result.sourceModelId === EXPECTED.sourceModelId, 'Derived A16 source model changed')
assert(asset.publicationEligible, 'Verified A16 source is not publication-eligible')
assert(asset.sourceKind === 'official-spigen-empty-case-image', 'A16 source kind changed')
assert(result.sourceKind === 'derived-official-source', 'A16 derived source kind changed')
assert(asset.catalogIdentityEvidence.verified, 'A16 canonical catalog identity is not verified')
assert(asset.catalogIdentityEvidence.matchedModel.name === EXPECTED.modelName, 'A16 canonical inventory name changed')
assert(asset.productRecordVerified && asset.repeatedFetchByteIdentical, 'Spigen source verification is incomplete')
...
assert(asset.officialDimensionEvidence.modelCode === EXPECTED.modelCode, 'Verified Samsung model changed')
assert(asset.officialDimensionEvidence.apiStatusCode === 200, 'Samsung API status changed')
assert(asset.officialDimensionEvidence.reportedValueFound && asset.officialDimensionEvidence.plmDimensionsVerified, 'Samsung dimensions are incomplete')
...
assert(result.sourceAsset.path === asset.path, 'A16 derivation source path changed')
assert(result.sourceAsset.encodedSha256 === asset.encodedSha256, 'A16 derivation source hash changed')
assert(result.sourceGeometry.primaryThreshold === EXPECTED.primaryThreshold, 'A16 primary threshold changed')
assertExactArray(result.sourceGeometry.stabilityThresholds, EXPECTED.thresholds, 'A16 threshold band')
assert(result.sourceGeometry.requiredMinimumThresholdIou === 0.995, 'A16 minimum IoU policy changed')
assert(result.sourceGeometry.allowedMaximumBoundsDrift === 4, 'A16 bounds drift policy changed')
assert(result.sourceGeometry.minimumThresholdIou >= result.sourceGeometry.requiredMinimumThresholdIou, 'A16 threshold IoU failed')
assert(result.sourceGeometry.maximumBoundsDrift <= result.sourceGeometry.allowedMaximumBoundsDrift, 'A16 threshold bounds drift failed')
assert(result.transform.spatialTransform === 'source-axis crop with fixed transparent padding only', 'A16 spatial transform changed')
assert(result.transform.sourceVisibleOpeningTransform.clearedPixels === 0, 'A16 source opening pixels were replaced')
assert(result.transform.morphologyOperations === 0, 'A16 morphology is forbidden')
assert(result.transform.inferredBoundaryPixels === 0, 'A16 boundary pixels were inferred')
assert(result.transform.inferredOpeningPixels === 0, 'A16 opening pixels were inferred')
assert(result.transform.sourceRgbUsed === false, 'A16 source RGB leaked into neutral candidates')
assert(result.alpha.cameraOpeningProfile.id === EXPECTED.openingProfile, 'A16 opening profile changed')
assert(result.alpha.cameraOpeningProfilePassed, 'A16 exact opening profile failed')
assert(result.alpha.unexpectedSignificantHoles.length === 0, 'A16 has an unexpected significant opening')
assert(result.alpha.significantHoles.length === 1, 'A16 must have exactly one significant connected opening')
assert(result.alpha.significantHoles[0].openingId === EXPECTED.openingId, 'A16 opening identity changed')

const finishes = new Map(result.candidates.map((candidate) => [candidate.finish, candidate]))
assert(finishes.size === 2 && finishes.has('black') && finishes.has('white'), 'Expected A16 Black and White outputs')
const black = finishes.get('black')
const white = finishes.get('white')
assert(black.outputAlphaSha256 === white.outputAlphaSha256, 'A16 Black and White alpha differs')
assert(black.outputAlphaSha256 === result.alpha.sha256, 'A16 candidate alpha differs from reviewed alpha')
...
assert(decodedBlack.alpha.equals(decodedWhite.alpha), 'A16 decoded pair alpha differs')
assert(decodedBlack.meanOpaqueLuminance < 70, 'A16 Black finish is too bright')
assert(decodedWhite.meanOpaqueLuminance > 220, 'A16 White finish is too dark')
...
assert(openings.length === 1, `Expected one decoded A16 opening, found ${openings.length}`)
assert(openings[0].pixels === result.alpha.significantHoles[0].pixels, 'Decoded A16 opening pixels changed')
...
if (overlayOnly) {
  ...
  const review = {
    ...
    acceptanceCriteria: [
      'The canonical case inventory explicitly defines galaxy-a16 as Galaxy A16 5G, matching the official Spigen variant and Samsung model.',
      'The official Spigen product, variant, SKU ACS08891, GTIN, media alt, and twice-fetched source bytes identify the complete real empty Galaxy A16 5G shell.',
      'Samsung identifies SM-A166BZKDEUB and independently reports 164.4 x 77.9 x 7.9 mm through the product page, model API, and three PLM dimension fields.',
    ...
    models: [{
      ...
      visualReview: {
        status: 'accepted',
        criteria: 'The hash-locked source overlay confirms a complete real shell outer boundary, all physical side structures, and the sole connected camera-and-flash opening with transparent margin on every side.',
