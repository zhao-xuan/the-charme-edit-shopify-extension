...
function buildCatalog() {
  ...
  const CHARMS = MERGED_CHARMS.map((c) => {
    ...
    // matched charms get the Shopify variant id (+ a fallback price); unmatched
    // charms are flagged unavailable so the tray greys them out. Applied LAST so
    // it wins even when the remote catalogue overrode the bundled charm by id.
    //
