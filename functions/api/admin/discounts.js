...
async function syncBundle(env, b) {
  ...
  // Anchor-first model: target (what it's for) + partner (what to bundle with).
  // Legacy bundles used main/match/items — fall back to those. The automatic
  // discount is scoped to ALL involved products/collections so whichever pair is
  // bought gets the saving.
  const target = b.target || b.main || { mode: 'products', products: b.items || [] }
  const partner = b.partner || b.match || { mode: 'products', products: [] }
  const prodHandles = []
  const colHandles = []
  for (const side of [target, partner]) {
    if (!side) continue
  ...
  if (!prodHandles.length && !colHandles.length) return { skip: 'no products/collections in the offer' }
  const pGids = []
  ...
  if (!pGids.length && !cGids.length) throw new Error('none of the offer products/collections were found in Shopify')
  const items = {}
